  ...
  if(args[0] == "kazma" || args[0] == "Kazma" || args[0] == "KAZMA")
  {
    ...
    let sipariş = args[1] * 2000;
      if(sipariş <= meh.get(`Para_${message.author.id}`))
         {
           ağırlık1 = 3.83 * args[1];
           ağırlıkk = ağırlıkk + Number(ağırlık1);
           ağırlık1 = 0;
           toplamağır = Number(ağırlıkk) + Number(meh.get(`ağırlık_${message.author.id}`));   
           if(Number(toplamağır) <= meh.get(`ağırgüç_${message.author.id}`)){
    ...
    message.channel.send(embd);
           ...
           message.channel.send(`> FAZLA AĞIR EŞYA ALAMAZSIN`);
         }
         }
      else
      {
