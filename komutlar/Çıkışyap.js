...
exports.run = async(client, message, args) => {
  ...
  var hesap = meh.get(`Banka_Hesap_${şifre2}`);
  
  if(hesap == null)return (message.channel.send(`> **ÇIKIŞ YAPICAK BİR HESAP YOK**`))
  const silmefiltresi = meh.get(`Kişiler_${hesap}_${şifre2}`).filter(x => x !== `${message.author.username}:${message.author.id}`);  
let arr = [];
